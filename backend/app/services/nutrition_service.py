...
def _find_food(name: str) -> dict | None:
    """同時對 name/canonical 做全相等與包含（模糊）比對"""
    _ensure_loaded()
    key = _norm(name)
    if not key:
        return None
    ...
    for r in _FOODS:
        nm = _col(r, NAME_KEYS, "")
        cano = str(r.get("canonical", "")).strip()
        if _norm(nm) == key or _norm(cano) == key:
            return r

    # 2) 模糊包含
    for r in _FOODS:
        collected = " ".join([
            str(_col(r, NAME_KEYS, "")) or "",
...
def calc(items: List[Dict], include_garnish: bool = True) -> Tuple[List[Dict], Dict]:
    """
