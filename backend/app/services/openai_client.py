...
def analyze_food_image(image_bytes: bytes) -> Dict[str, Any]:
    """
    Calls OpenAI Vision to identify foods and estimate grams per item.
    Returns a dict with 'items': [{name, grams, confidence}]
    """
    ...
    prompt = (
        ...
        "並用最接近台灣常見菜名表示，例如：白飯、雞胸肉、青花菜、滷肉飯、滷味拼盤、便當炸雞塊等。"
    ...
    try:
        ...
        items = data.get("items") or data  # allow array or object with items
        # Normalize
        clean = []
        for it in items:
